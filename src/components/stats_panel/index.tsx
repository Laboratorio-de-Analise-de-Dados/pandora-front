...
import { findGateInTree, collectAllGates, findFileForGate, getGateStrategy } from "../../features/gate/utils"
...
const METRIC_COLUMNS: MetricDef[] = [
	{ key: "mean_mfi", label: "MFI (Mean)", shortLabel: "Mean", format: (v) => v.toLocaleString(undefined, { maximumFractionDigits: 1 }) },
	{ key: "median_mfi", label: "MFI (Median)", shortLabel: "Median", format: (v) => v.toLocaleString(undefined, { maximumFractionDigits: 1 }) },
	{ key: "std_dev", label: "Std Dev", shortLabel: "SD", format: (v) => v.toLocaleString(undefined, { maximumFractionDigits: 1 }) },
	{ key: "cv", label: "CV (%)", shortLabel: "CV", format: (v) => `${v.toFixed(1)}%` },
]
...
export default function StatsPanel({
	...
	useEffect(() => { localStorage.setItem(LS_KEY_LABELS, JSON.stringify(customLabels)) }, [customLabels])
	...
	const allChannels = useMemo(() => {
		...
		if (analysis?.channel_statistics) return Object.keys(analysis.channel_statistics)
		if (fileStats?.channel_statistics) return Object.keys(fileStats.channel_statistics)
	...
	useEffect(() => {
		try {
			...
			if (saved) { setSelectedChannels(new Set(JSON.parse(saved) as string[])); return }
		} catch { /* ignore */ }
		setSelectedChannels(new Set(allChannels))
	}, [])

	useEffect(() => {
		if (selectedChannels === null && allChannels.length > 0) setSelectedChannels(new Set(allChannels))
	}, [allChannels, selectedChannels])

	useEffect(() => { if (selectedChannels) localStorage.setItem(LS_KEY_CHANNELS, JSON.stringify([...selectedChannels])) }, [selectedChannels])
	useEffect(() => { localStorage.setItem(LS_KEY_METRICS, JSON.stringify([...visibleMetrics])) }, [visibleMetrics])

	const toggleChannel = useCallback((ch: string) => {
		setSelectedChannels((prev) => { const next = new Set(prev); if (next.has(ch)) next.delete(ch); else next.add(ch); return next })
	}, [])

	const toggleMetric = useCallback((m: MetricColumn) => {
		setVisibleMetrics((prev) => { const next = new Set(prev); if (next.has(m)) next.delete(m); else next.add(m); return next })
	}, [])

	const selectAll = useCallback(() => setSelectedChannels(new Set(allChannels)), [allChannels])
	const clearAll = useCallback(() => setSelectedChannels(new Set()), [])
	const selectFluorescence = useCallback(() => setSelectedChannels(new Set(allChannels.filter(isFluorescence))), [allChannels])
	...
	const analysisData: AnalysisResultData | undefined = useMemo(() => {
		if (currentGate?.analysis_result?.analysis_result) return currentGate.analysis_result.analysis_result
	...
	const handleExport = useCallback(
		(scope: "current" | "all", format: "csv" | "xlsx") => {
			...
			setExportDialog({ open: true, defaultName: baseName, format, handler: (fileName, fmt) => exportRows(rows, `${fileName}.${fmt}`, fmt) })
		},
	...
	if (!source) {
		return (
			<Box sx={{ p: 2 }}>
				{headerSection}
				<SourceSelector currentPath={currentPath} selectableItems={selectableItems} source={source} onSelect={setStatsSource} />
		...
		return (
			<Box sx={{ p: 2 }}>
				...
				<Box sx={{ p: 1.5, bgcolor: "action.hover", borderRadius: 1, mt: 0.5 }}>
					<Typography variant="body2" color="text.secondary">Canais disponíveis:</Typography>
					<Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5, mt: 0.5 }}>
						{values.map((v) => <Chip key={v} label={v} size="small" variant="outlined" />)}
					</Box>
		...
		<Box sx={{ p: 1.5, display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
			...
			<Box sx={{ mt: 0.5, border: "1px solid", borderColor: "divider", borderRadius: 1, overflow: "hidden" }}>
				<Box
					sx={{
						...
						bgcolor: "action.hover", cursor: "pointer", "&:hover": { bgcolor: "action.selected" },
					}}
					onClick={() => setStatsExpanded((p) => !p)}
				>
					{statsExpanded ? <ExpandLessIcon style={{ fontSize: 16 }} /> : <ExpandMoreIcon style={{ fontSize: 16 }} />}
							...
							<>
								<Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 0.5 }}>
									<Tooltip title="Configurar parâmetros e colunas">
										<IconButton size="small" onClick={(e) => setChannelMenuAnchor(e.currentTarget)} sx={{ p: 0.25 }}>
			...
			{!channelStats && source.type === "gate" && (
				<Box sx={{ textAlign: "center", py: 3 }}>
					<Typography variant="body2" color="text.secondary">Calculando estatísticas...</Typography>
					<Typography variant="caption" color="text.secondary">As métricas serão exibidas em instantes.</Typography>
				</Box>
			)}
			...
			<ExportDialog
				...
				onFormatChange={(fmt) => setExportDialog((p) => ({ ...p, format: fmt }))}
				onExport={() => {
